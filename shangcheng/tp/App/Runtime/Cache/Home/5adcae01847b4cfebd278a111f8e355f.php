		...
		<div class="content full-content clearfix">
			...
			<div class="product-menu-holder"><!-- 菜单开始-->
					<div class="table-operate ue-clear">
					    <a href="<?php echo U('Index/index');?>"  class="add">添加</a>
		                ...
		                <?php if(is_array($foods)): $i = 0; $__LIST__ = $foods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fo): $mod = ($i % 2 );++$i;?><tr>
			                    ...
			                    <td class="category_id"><?php echo ($fo["category_id"]); ?></td>
			                   	<td class="num">
				                   	...
				                   	<button onclick="minus(this)" data="<?php echo ($fo["id"]); ?>" > - </button>
				                   	<button ><?php echo ($fo["num"]); ?></button>
				                   	<button onclick="plus(this)" data="<?php echo ($fo["id"]); ?>" > + </button>
...
<script type="text/javascript">
 	...
 	function minus(a){
		...
		if(num >= 1){
			...
			var id = $(a).attr('data');
			//alert(id);die;
			window.location.href="/index.php/Home/Cart/editMinusNum/id/"+id+"/num/"+num;
	    }
	}
	function plus(a){
		var num = $(a).prev().html();
		...
		var id = $(a).attr('data');
		//alert(id);die;
		window.location.href="/index.php/Home/Cart/editPlusNum/id/"+id+"/num/"+num;
	}
$(function(){
	    //给删除按钮添加点击事件
	    //获取选中的值，可以同时删除多个
	    $('#btndel').on('click',function(){
	        ...
	        for(var i = 0;i < ids.length;i++){
	            ...
	            //alert(id);
	            window.location.href = '/index.php/Home/Cart/delcart/id/' + id;
	    }); 
	    $('#submit').on('click',function(){
	    	//alert('1111');die;
	    	window.location.href = '/index.php/Home/Cart/order';
	    });
}); 
